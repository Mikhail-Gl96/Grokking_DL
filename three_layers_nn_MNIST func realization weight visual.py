...
def nn(input, weight, goal_prediction, alpha, to_visual=0, train_acc=0):
    print_star_num = 80
    ...
    if to_visual == 1:
        ...
        print("*" * print_star_num)
    train_acc = get_train_acc(train_acc=train_acc, output=layer_2, goal_prediction=goal_prediction)
...
for j in range(iterations):
    ...
    for i in range(len(images)):
        ...
        results_my = nn(input=input, weight=weight, goal_prediction=goal_prediction, alpha=alpha, to_visual=to_visual, train_acc=train_acc)
    ...
    if (j % 10 == 0) or (j == iterations - 1):
        ...
        for i in range(len(test_images)):
            ...
            label_data = np.argmax(test_labels[i:i + 1])
            weight_vis = np.array(layer_l_t.copy()).reshape(28, 28)
            # print(f'iter = {i}')
            # data = layer_0_t
            # print('\n', weights_0_1.shape, '\n', arr_print(weights_0_1))
            # time.sleep(20)
        sys.stdout.write("   Test-Err:" + str(error_t / float(len(test_images)))[0:5] + " Test-Acc:" + str(correct_cnt_t / float(len(test_images))))
        print()
        # print('\n', weight_vis.shape, '\n', arr_print(weight_vis))
        im = Image.fromarray(np.uint8(weight_vis*255), 'L')
        # im.show()
        # im.close()
        #im = Image.fromarray(np.uint8(data*255))
        # im.show()
        im.save(f"pics/pic_{j}___ans_{label_data}.png")
